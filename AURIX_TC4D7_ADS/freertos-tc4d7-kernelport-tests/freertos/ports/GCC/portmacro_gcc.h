...
#define portSRC_SRCR_VM_OFF      8
#define portSRC_SRCR_SRE_OFF     23
#define portSRC_SRCR_TOS_OFF     12
#define portSRC_SRCR_SRR_OFF     24
#define portSRC_SRCR_CLRR_OFF    25
 ...
 */
static inline void __attribute__( ( used, always_inline ) ) vPortSetCCPN( unsigned char ucCCPN )
{
    _disable();
...
static inline void __attribute__( ( used, always_inline ) ) vPortSetICR( uint32_t ulICR )
{
    _disable();
    _mtcr( portCPU_ICR, ulICR );
    _isync();
    _enable();
}

static inline uint32_t __attribute__( ( used, always_inline ) ) xPortSetCCPN( unsigned char ucCCPN )
{
    uint32_t ulICR;

    _disable();
    ulICR = _mfcr( portCPU_ICR );
    _mtcr( portCPU_ICR, ( ulICR & ~portCPU_ICR_CCPN_MSK ) | ( ucCCPN & portCPU_ICR_CCPN_MSK ) );
    _isync();
    _enable();
    return ulICR;
}
