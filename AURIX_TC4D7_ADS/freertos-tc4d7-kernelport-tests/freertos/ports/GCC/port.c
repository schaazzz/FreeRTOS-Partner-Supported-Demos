...
#define portTICK_COUNT    ( configCPU_STM_CLOCK_HZ / configTICK_RATE_HZ )

/* Register defines */
static volatile uint32_t *const pxCPU = configCPU;
static volatile uint32_t *const pxCPUSrc = configCPU_STM_SRC;
static volatile uint32_t *const pxContextSrc = configCONTEXT_SRC;
#define portSTM_OCS                  0x04
#define portSTM_CMP0                 ( 0x100 + configVM_NR * 0x20 )
#define portSTM_COMCON               ( 0x108 + configVM_NR * 0x20 )
#define portSTM_ICR                  ( 0x10C + configVM_NR * 0x20 )
#define portSTM_ISCR                 ( 0x110 + configVM_NR * 0x20 )
#define portSTM_TIM0                 ( 0x118 + configVM_NR * 0x20 )

#define portSTM_CMCON_MSTART0_OFF    8
#define portSTM_CMCON_MSIZE0_OFF     0
#define portSTM_ICR_CMP0EN_OFF       0
#define portSTM_ICR_CMP0OS_OFF       1
...
void __attribute__( ( interrupt_handler ) ) vPortSystemTickHandler()
{
    ...
    pxCPU[ portSTM_CMP0 >> 2 ] = pxCPU[ portSTM_CMP0 >> 2 ] + portTICK_COUNT;
    pxCPU[ portSTM_ISCR >> 2 ] |= ( 1 << portSTM_ISCR_CMP0IRR_OFF );
     ...
     * counts missed.   */
        ...
        configASSERT( ( pxCPU[ portSTM_CMP0 >> 2 ] - pxCPU[ portSTM_TIM0 >> 2 ] ) <= portTICK_COUNT );
    #endif
...
{
    pxCPU[ portSTM_COMCON >> 2 ] =
        ( 0 << portSTM_CMCON_MSTART0_OFF ) | ( 31 << portSTM_CMCON_MSIZE0_OFF );
    pxCPU[ portSTM_ICR >> 2 ] &= ~( 1 << portSTM_ICR_CMP0OS_OFF );
    pxCPUSrc[ 0 ] = ( ( configCPU_NR ) << portSRC_SRCR_TOS_OFF ) |
                    ( ( configVM_NR ) << portSRC_SRCR_VM_OFF ) |
                    ( ( configTIMER_INTERRUPT_PRIORITY ) << portSRC_SRCR_SRPN_OFF );
    pxCPUSrc[ 0 ] |= ( 1 << portSRC_SRCR_SRE_OFF );
    pxCPU[ portSTM_CMP0 >> 2 ] = pxCPU[ portSTM_TIM0 >> 2 ];
    pxCPU[ portSTM_ISCR >> 2 ] |= ( 1 << portSTM_ISCR_CMP0IRR_OFF );
    pxCPU[ portSTM_ICR >> 2 ] |= ( 1 << portSTM_ICR_CMP0EN_OFF );
    pxCPU[ portSTM_CMP0 >> 2 ] = pxCPU[ portSTM_TIM0 >> 2 ] + portTICK_COUNT;
    #if configTICK_STM_DEBUG != 0
        pxCPU[ portSTM_OCS >> 2 ] = 0x12000000;
    #endif
}
...
{
    pxContextSrc[ 0 ] =
        ( ( configCPU_NR ) << portSRC_SRCR_TOS_OFF ) |
        ( ( configVM_NR ) << portSRC_SRCR_VM_OFF ) |
...
#endif // #if defined(__GNUC__) && !defined(__HIGHTEC__)
